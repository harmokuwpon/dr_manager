...
for l in range(1,2):
    ...
    for i in range(1,search_result+1):
        ...
        try:
                    ...
                    themename= driver.find_element_by_xpath(xpath).get_attribute("value") #エラーがでやすい
                    ...
                    try:
                        if len(driver.find_element_by_xpath(comment_No_string).text)!=0:
                         ...
                         date_sub=datetime.timedelta(days=10000) #データがない場合には100日を仮入力とする
                    ...
                    if (date_sub.days<8): #一週間以内のデータを抽出
                        ...
                        for k in range(1,6):
                                    ...
                                    for j in range(3,16):
                                        ...
                                        if k==4 :
                                         ws.cell(row=(5*(i-1)+k+2),column=7,value="見込み")#見込み　ヘッドレスモードになると消えるため
                                        elif k==5:
                                         ws.cell(row=(5*(i-1)+k+2),column=7,value="実績")#実績　　　ヘッドレスモードになると消えるため



                                        ws.cell(row=1,column=1,value="抽出結果"+str(extract_result)+"件") #検索件数
                                        ws.cell(row=1,column=3,value="プログラム実施日"+str(dt)) #検索件数
        ...
        #行削除
                ws.delete_rows(i)
        if (extract_result==0): #一週間以内のデータを抽出
                            ws.cell(row=1,column=1,value="抽出結果"+str(extract_result)+"件") #検索件数
                            ws.cell(row=1,column=3,value="プログラム実施日"+str(dt)) #検索件数

        wb.save('C:\\Users\\010020035\\OneDrive - OMRON\\py\\DR Manger(コメントから抽出).xlsx')#ONEDRIVEに保存
